...
<?php
          ...
          if(isset($_GET['submit_range']) and ($_GET['min']) and ($_GET['max']))
{    
	
     $min=$_GET['min'];
     $max=$_GET['max'];    
    
              $result = $produitC-> filtre_prix_produit($_GET['min'],$_GET['max']);
?>
              <?PHP
...
<div class="form-price-range-filter">
    <form method="post" action="filtreprix.php">
        <div>
            MIN:<input type="number" id="min" name="min" min="0">
            <div id="slider-range"></div>
           MAX: <input type="number" id="max" name="max" min="0">
        </div>
        <div>
            <input class="supFav" type="submit" name="submit_range"
